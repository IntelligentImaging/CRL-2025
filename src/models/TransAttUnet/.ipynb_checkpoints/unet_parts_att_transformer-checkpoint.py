...
class PositionEmbeddingLearned(nn.Module):
    ...
    def __init__(self, num_pos_feats=256, len_embedding=32):
        super().__init__()
        self.row_embed = nn.Embedding(len_embedding, 85)
        self.col_embed = nn.Embedding(len_embedding, 85)
        self.depth_embed = nn.Embedding(len_embedding, 86)  # Added depth embedding
        self.reset_parameters()

    def reset_parameters(self):
        nn.init.uniform_(self.row_embed.weight)
        nn.init.uniform_(self.col_embed.weight)
        nn.init.uniform_(self.depth_embed.weight)  # Initialize depth embedding

    def forward(self, tensor_list):
        x = tensor_list
        d, h, w = x.shape[-3:]  # Updated to include depth
        i = torch.arange(w, device=x.device)
        j = torch.arange(h, device=x.device)
        k = torch.arange(d, device=x.device)  # Depth indices

        x_emb = self.col_embed(i)
        y_emb = self.row_embed(j)
        z_emb = self.depth_embed(k)  # Depth embedding
