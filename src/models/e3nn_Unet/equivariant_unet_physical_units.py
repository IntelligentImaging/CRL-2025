...
class ConvolutionBlock(nn.Module):
    def __init__(self, input, irreps_hidden, activation, irreps_sh, normalization, diameter, num_radial_basis, steps,
                 dropout_prob, cutoff):
        super().__init__()
        ...
        self.conv2 = Convolution(self.gate1.irreps_out, self.gate2.irreps_in, irreps_sh, diameter, num_radial_basis,
                                 steps, cutoff=cutoff)
...
class Down(nn.Module):
    ...
    def __init__(self, n_downsample, activation, irreps_sh, ne, no, BN, input, diameters, num_radial_basis, steps,
                 down_op, scale, stride, dropout_prob, cutoff):
        super().__init__()
        ...
        for n in range(n_downsample + 1):
            irreps_hidden = Irreps(
                f"{4 * ne}x0e + {4 * no}x0o + {2 * ne}x1e +  {2 * no}x1o + {ne}x2e + {no}x2o").simplify()
            block = ConvolutionBlock(input, irreps_hidden, activation, irreps_sh, BN, diameters[n], num_radial_basis,
                                     steps[n], dropout_prob, cutoff)
            blocks.append(block)
...
class Up(nn.Module):
    def __init__(self, n_blocks_up, activation, irreps_sh, ne, no, BN, downblock_irreps, diameters, num_radial_basis,
                 steps, up_op, scale, stride, dropout_prob, scalar_upsampling, cutoff):
        super().__init__()
        ...
        for n in range(n_blocks_up):
            if scalar_upsampling:
                ...
                irreps_hidden = Irreps(
                    f"{4 * ne}x0e + {4 * no}x0o + {2 * ne}x1e + {ne}x2e + {2 * no}x1o + {no}x2o").simplify()

            block = ConvolutionBlock(input + downblock_irreps[::-1][n + 1], irreps_hidden, activation, irreps_sh, BN,
                                     diameters[n], num_radial_basis, steps[n], dropout_prob, cutoff)
            blocks.append(block)
            input = block.irreps_out
            ne //= 2
            no //= 2
            ...
            upsample_scale_factor = tuple(
                [math.floor(scale[n] / step) if step < scale[n] else 1 for step in steps[n]])  # same as pooling kernel
...
class UNet(SegmentationNetwork):
    def __init__(self, input_irreps, output_irreps, diameter, num_radial_basis, steps, batch_norm='instance', n=2,
                 n_downsample=2, equivariance='SO3',
        ...
        assert down_op in ['maxpool3d', 'average',
                           'lowpass'], "down_op needs to be 'maxpool3d', 'average', or 'lowpass'"
        ...
        self.down = Down(n_downsample, activation, irreps_sh, ne, no, batch_norm, input_irreps, diameters,
                         num_radial_basis, steps_array, down_op, scales, stride, dropout_prob, cutoff)
        ne *= 2 ** (n_downsample - 1)
        no *= 2 ** (n_downsample - 1)
        self.up = Up(n_downsample, activation, irreps_sh, ne, no, batch_norm, self.down.down_irreps_out,
                     diameters[::-1][1:], num_radial_basis, steps_array[::-1][1:], up_op, scales[::-1], stride,
                     dropout_prob, scalar_upsampling, cutoff)
